...
# class SiTBlock_OS(nn.Module):
#     """
#     A SiT block without adaptive layer norm zero (adaLN-Zero) conditioning.
#     """
#     def __init__(self, hidden_size, num_heads, mlp_ratio=4.0, **block_kwargs):
#         super().__init__()
#         self.norm1 = nn.LayerNorm(hidden_size, elementwise_affine=False, eps=1e-6)
...
class SiTOS(nn.Module):
    ...
    def __init__(
        ...
        **block_kwargs # fused_attn
        ...
        self.visual_proj.requires_grad_(False)
        ...
        # self.input_layer = nn.Linear(embed_size, hidden_size, bias=True)
        self.final_layer = FinalLayer(hidden_size, embed_size)
        self.initialize_weights()
    ...
    def forward(self, x, t):
        ...
        for i, block in enumerate(self.blocks):
            ...
            #     zs = self.projector(x.mean(dim=1))
                # zs = x
...
class CFSiT(nn.Module):
    ...
    def __init__(
        ...
        **block_kwargs # fused_attn
        ...
        #     nn.Linear(hidden_size, hidden_size, bias=True),
        # )
        # self.input_proj = nn.Linear(hidden_size, hidden_size, bias=True)
        self.variational_encoder = VariationalEncoder()
        self.visual_proj = visual_proj
        self.visual_proj.requires_grad_(False)
        # self.ClipVAE = CLIPVAE()
        # 可训练的缩放参数，作用于 clip 特征
    ...
    def forward(self, x, t):
        ...
        # x = self.input_proj(x) + self.pos_embed  # (N, P, D)
        x = x + self.pos_embed  # (N, P, D)
        c = self.t_embedder(t)                              # (N, D)
        for i, block in enumerate(self.blocks):
            x = block(x, c)                      # (N, T, D)
...
#         self.blocks = nn.ModuleList([
#             SiTBlock_OS(hidden_size, num_heads, mlp_ratio=mlp_ratio, **block_kwargs) for _ in range(depth)
#         ])
#         self.initialize_weights()
